...
export default function Dashboard() {
         ...
         parkingsRef
            ...
            .then((doc) => {
               if (!doc.exists) {
                  ...
                  calculateTime(
                     doc.data().reservationsTime,
                     doc.data().reservationsFinished
                  );
               }
            });
   ...
   async function checkAverageTime() {
      ...
      if (typeof log.data() !== "undefined") {
         ...
         calculateTime(
            log.data().reservationsTime,
            log.data().reservationsFinished
         );
      } else {
         setAverageTime("Unavailable");
               ...
               <div className="dashboard-card">
                  ...
                  <div className="label">
                     {parking ? (
                        !loadingSpots ? (
                           <h1>
                              {availableSpots} / {parking.totalSpots}
                           </h1>
                        ) : (
                           <h1>
                              <ReactLoading
